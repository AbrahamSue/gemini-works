...
(async () => {
    ...
    try {
        ...
        // Output logic: Always stdout the dlm3u8 command line
        // Return full metadata as stderr if verbose
        
        return {
            _stdout: cmd,
            _stderr: typeof inVerbose !== 'undefined' && inVerbose ? JSON.stringify(result, null, 2) : ""
        };
